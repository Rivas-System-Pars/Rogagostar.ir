...
use App\Http\Controllers\Back\CustomerClubController;
...
Route::get('/migrate', function () {
	...
	$user = User::first();
	$user->username = "09115059242";
	$user->notify(new InstallmentNotification());

   SendInstallmentSms::dispatch(Order::latest()->has('installment')->first()->installment->items->first()->id); dd(is_numeric(trim(option('installment_before_day'))),is_numeric(trim(option('installment_day'))),is_numeric(trim(option('installment_after_day'))));
});
...
Route::group(['as' => 'admin.', 'prefix' => 'admin/' . admin_route_prefix(), 'middleware' => ['auth', 'Admin', 'verified', 'CheckPasswordChange', 'password.confirm']], function () {
	...
	Route::resource('sales-agency', SalesAgencyController::class)->only(['index','show']);
	Route::get('sales-agency/{id}/download', [SalesAgencyController::class,'download'])->name('sales-agency.download');
	Route::resource('careeropportunities', CareeropportunitiesController::class)->only(['index','show']);
	Route::get('careeropportunities/{id}/download', [CareeropportunitiesController::class,'download'])->name('careeropportunities.download');
	//Route::resource('customer-club', CustomerClubController::class)->only(['index','show']);
	Route::get('customer-club/index', [CustomerClubController::class , 'index'])->name('CustomerClub.index');
	Route::get('customer-club/show/{id}', [CustomerClubController::class , 'show'])->name('CustomerClub.show');

	//Route::resource('customer-club', CustomerClubController::class)->only(['index','show']);

	Route::resource('counseling-form', CounselingFormController::class)->only(['index','show','destroy']);
