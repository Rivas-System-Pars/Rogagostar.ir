    ...
    <div class="product-info dt-sl">
        ...
        <div class="row pt-4">
            ...
            <div class="col-md-6">
				...
				@else
					@php
                        $selected_price = $product->prices()->first();
                    @endphp
                @endif
            </div>
        </div>
...
{{--
		...
		<div id="rating-container">
         ...
         @endphp
            ...
            {{-- <span class="price d-flex justify-content-center align-items-center" style="margin-left: 450px;">
            {{-- <div id="stars" data-auth="{{ auth()->check() ? 'true' : 'false' }}">
                @for ($i = 1; $i <= $filledStars; $i++)
                    <i class="fa-solid fa-star star" data-value="{{ $i }}"
                        style="color: gold; cursor: {{ auth()->check() ? 'pointer' : 'default' }}; font-size:25px;"></i>
                @endfor
                @for ($i = $filledStars + 1; $i <= 5; $i++)
                    <i class="fa-solid fa-star star" data-value="{{ $i }}"
                        style="color: gray; cursor: {{ auth()->check() ? 'pointer' : 'default' }}; font-size:25px;"></i>
                @endfor
            </div> --}}
            {{-- <div>
                <p id="average-rating-text" class="my-0" style="font-weight: 400;">
                    امتیاز <span id="average-rating">{{$product->avg_rating_star ? $product->avg_rating_star : '0'}}</span> از 5
                </p>
            </div> --}}
         </span>
    </div>
            ...
            @if($product->addableToCart())
                ...
                {{-- <div class="mb-2 d-inline">
                    ...
                    <div class="number-input">
                        ...
                        <input id="cart-quantity" class="quantity" min="{{ $product->isDownload() ? 1 : cart_min($selected_price) }}" max="{{ $product->isDownload() ? 1 : cart_max($selected_price) }}" value="{{ $product->isDownload() ? 1 : cart_min($selected_price) }}" type="number" required>
                        <button type="button" onclick="this.parentNode.querySelector('input[type=number]').stepUp()" class="plus"></button>
                    </div>
                </div> --}}

                <a href="tel:01333530946 ">
                    <button type="button" class="btn-primary-cm btn-with-icon ">

                            برای اطلاع از موجودی و شرایط خرید تماس بگیرید
                        </button>
                    </a>
